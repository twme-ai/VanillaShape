...
final class ShapeRenderer {
    ...
    ShapeRenderer(final ClientBlockStore store) {
        this.store = store;
    }

    void render(final WorldRenderContext context) {
        final ClientLevel level = context.world();
        final PoseStack poseStack = context.matrixStack();
        final MultiBufferSource consumers = context.consumers();
        if (level == null || poseStack == null || consumers == null) return;
        final Vec3 camera = MinecraftCompat.cameraPosition(context.camera());
        final String world = MinecraftCompat.worldId(level);
        for (final SpecialBlock block : store.blocks(world)) {
            ...
            final VertexConsumer buffer = consumers.getBuffer(renderType);
            poseStack.pushPose();
            poseStack.translate(block.x() - camera.x, block.y() - camera.y, block.z() - camera.z);
            draw(block, material, poseStack.last(), buffer, light, level, pos);
            poseStack.popPose();
        }
    }
    ...
    private void draw(final SpecialBlock block, final ModelMaterialResolver.Resolved material,
        ...
        for (final ShapeGeometry.Surface surface : ShapeGeometry.surfaces(block)) {
            final BlockPos neighbor = pos.relative(surface.direction());
            if (boundary(surface) && MinecraftCompat.isSolidRender(level.getBlockState(neighbor), level, neighbor)) {
                continue;
            }
    ...
    private void drawModel(final SpecialBlock block, final ModelMaterialResolver.Resolved material,
        ...
        for (final TemplateModelResolver.Quad quad : models.resolve(block.model())) {
            if (quad.cullDirection() != null) {
                final BlockPos neighbor = pos.relative(quad.cullDirection());
                if (MinecraftCompat.isSolidRender(level.getBlockState(neighbor), level, neighbor)) continue;
            }
